...
Cda.Cdb1.v = soft_centroid_vector(X, y, alpha);
...
while iter <= parameter.maxIter
    ...
    alpha = update_sample_weight(alpha, X, y, Cda.Cdb1, parameter);

    % compute the soft-centroid vector CDB2; find the orthogonal vector to
    % CDB1
    Cda.Cdb2.v = soft_centroid_vector(X, y, alpha);
    Cda.Cdb2.v_orth = normalize(Cda.Cdb2.v - Cda.Cdb1.v * Cda.Cdb2.v' * Cda.Cdb1.v, "norm");
...
function v = soft_centroid_vector(X, y, alpha)
sign_map = [-1;1];
...
v = v / norm(v);    % CDA eigenvector is the unit vector pointing from the centroid of negative class to that of positive class.
end


function alpha = update_sample_weight(alpha, X, y, Cdb, parameter)
Q = X * Cdb.v';
...
Q_Cdb1 = X * Cda.Cdb1.v';
Q_Cdb2_orth = X * Cda.Cdb2.v_orth';
for i = 1:parameter.N_null_model
    Cdb.Q = cosd(theta(i)) * Q_Cdb1 +  sind(theta(i)) * Q_Cdb2_orth;
    Cdb = train_oop_cv(Cdb, y, parameter); 
    ps_list(i) = Cdb.ps;
    if Cdb.ps > best_ps
        best_random_Cdb = Cdb;
...
if ~isfield(parameter, "metrics"),                      parameter.metrics=["AUC","AUPR","Fscore","ACscore","acc"]; end
